...
async function removeDuplicateTabs(windowId) {
  try {
    ...
    for (const [, sameUrlTabs] of urlToTabs) {
      ...
      if (!keep)
        keep = unpinnedTabs.slice().sort((a, b) => a.index - b.index)[0];
      for (const t of unpinnedTabs) {
  ...
  try {
    ...
    let targetIndex =
      sameDomainTabs.length > 0
        ? Math.min(...sameDomainTabs.map((t) => t.index))
        : -1;
...
async function sortUngroupedTabs(unpinnedTabs, pinnedCount) {
  ...
  const alreadySorted = currentOrderIds.every(
    (id, i) => id === sortedOrderIds[i],
  );
